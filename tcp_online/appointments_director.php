...
function generate_apt_table($data){
	for($i = 0; $i < sizeof($data); $i++){
		...
		echo('
			<ul>
				<li><strong>' . tcp_date::raw_to_readable($data[$i]->get_apt_date()) . '</strong> - <span style="color: black">' . tcp_date::index_to_str($data[$i]->get_time_index()). '</span></li><br />
				<li type="square" style="margin-left: 5%">Fellow: <a href="mailto:' . $fellow->get_email() . '">' . $fellow->get_firstname() . ' ' . $fellow->get_lastname() . '</a></li>
				<li type="square" style="margin-left: 5%">Student: <a href="mailto:' . $data[$i]->get_email() . '">' . $data[$i]->get_first_name() . ' ' . $data[$i]->get_last_name() . '</a></li>
				<li type="square" style="margin-left: 5%">Pennkey: ' . $data[$i]->get_pennkey() . '</li>
				<li type="square" style="margin-left: 5%">Booking comment: ' . $data[$i]->get_comment() . '</li><br />
				<li type="square" style="margin-left: 5%"><a href="view_appointment.php?id=' . $data[$i]->get_id() . '">View Appointment</a></li>
			</ul>
		');
	}
}
...
<h1 align="left">All TCP Appointments</h1> 
...
if(sizeof($upcoming_appointments) == 0)//PHP sizeof gives array length
	echo('<li><span style="color: gray">There are no upcoming TCP appointments.</span></li>');
else
...
<ul> 
  ...
  <li class ="current sub noicon " ><a href="view_all_appointments.php">View All Appointments</a></li>
